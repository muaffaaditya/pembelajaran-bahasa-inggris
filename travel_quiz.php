...
<style>
  ...
  .container {
    ...
    border-radius: 20px;
    padding: 30px 40px 60px 40px; /* beri ruang bawah untuk tombol */
...
<script>
  // Soal teks vocab tentang travel
  const vocabQuestions = [
    {
      word: "What do you call a person who plans and organizes trips?",
      options: ["Tour Guide", "Pilot", "Chef"],
      answerIndex: 0
    },
    {
      word: "Which place is best known for its Eiffel Tower?",
      options: ["Rome", "Paris", "Berlin"],
      answerIndex: 1
    },
    {
      word: "What is the term for a place where travelers stay overnight?",
      options: ["Museum", "Hotel", "Airport"],
      answerIndex: 1
    }
  ];

  // Soal gambar tentang travel
const imageQuestions = [
  {
    question: "Choose the picture that shows a famous landmark in Egypt",
    images: [
      { src: "https://upload.wikimedia.org/wikipedia/commons/e/e3/Kheops-Pyramid.jpg", isAnswer: true, alt: "Pyramids of Giza" },
      { src: "https://upload.wikimedia.org/wikipedia/commons/a/a1/Statue_of_Liberty_7.jpg", isAnswer: false, alt: "Statue of Liberty" },
        { src: "https://cdn-icons-png.flaticon.com/512/1053/1053361.png", isAnswer: false, alt: "Barista" }
    ]
  },
  ...
  function showImageQuestion(index) {
    const q = imageQuestions[index - vocabQuestions.length];
    let html = `<div class="question"><strong>${q.question}</strong></div><div class="image-options">`;
    q.images.forEach((img, i) => {
      html += `
        <div class="image-option" tabindex="0" role="button" aria-label="${img.alt}" data-index="${i}">
    ...
    imgOptions.forEach(option => {
      option.addEventListener('click', () => {
        ...
        if (q.images[idx].isAnswer) {
          option.classList.add('correct');
          score++;
        } else {
          option.classList.add('wrong');
          // Highlight correct answer
          imgOptions.forEach((el, i) => {
            if (q.images[i].isAnswer) el.classList.add('correct');
          });
        }
        disableOptions();
        clearInterval(timerInterval);
  ...
  function showStep(step) {
    resetState();
    if (step < vocabQuestions.length) {
      showVocabQuestion(step);
    } else if (step < totalSteps) {
      showImageQuestion(step);
    } else {
      showScore();
    }
    startTimer();
